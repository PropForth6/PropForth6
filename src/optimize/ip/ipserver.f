...
h_02_01	wconstant	$S_ip_gatewaylo
]
\
						\ 255.255.255.0
...
\ client connect once off, client connect off, defer disconnect off, noexpandcr on,  reset
	>r -1 h0F h0F h0E h13 h0C h0A h08 h06 h04 h02 r> dup >r ip_SOCKsendmultiplereq r> swap
	if
		ip_SOCKstatus@ 0=
	else
		drop
...
\ 							\ telnet protocol on, noexpandcr on, server connect on
		>r -1 h07 h13 h03 r> ip_SOCKsendmultiplereq
...
: _ip_sockstat1
	...
	_ip_sir@ dup 0<> if
		hex
		." INTERRUPTS: " dup .byte 
		dup 1  and if ."  CON     " then
		dup h2  and if ."  DISCON  " then
		dup h4  and if ."  RECV    " then
		dup h8  and if ."  TIMEOUT " then
		    h10 and if ."  SEND_OK " then
		decimal
	else
		."                         "
		drop
...
: _ip_sockstat2
	>dbg
	hex
	_ip_ssr@ ."  STATUS: " dup .byte
	c"                  "
	over h1B <= if
		over 0= if  drop c"  SOCK_CLOSED     " else
		over h13 = if drop c"  SOCK_INIT       " else
		over h14 = if  drop c"  SOCK_LISTEN     " else
		over h15 = if  drop c"  SOCK_SYNSENT    " else
		over h16 = if  drop c"  SOCK_SYNRECV    " else
		over h17 = if  drop c"  SOCK_ESTABLISHED" else
		over h18 = if  drop c"  SOCK_FIN_WAIT   " else
		over h1A = if  drop c"  SOCK_CLOSING    " else
		over h1B = if  drop c"  SOCK_TIME_WAIT  " 
			  then then then then then then then then then
	else
		over h1C = if  drop c"  SOCK_CLOSE_WAIT " else
		over h1D = if  drop c"  SOCK_LAST_ACK   " else
		over h11 = if  drop c"  SOCK_ARP        " else
		over h21 = if  drop c"  SOCK_ARP        " else
		over h22 = if  drop c"  SOCK_UDP        " else
		over h31 = if  drop c"  SOCK_ARP        " else
		over h32 = if  drop c"  SOCK_IPRAW      " else
		over h42 = if  drop c"  SOCK_MACRAW     " else
		over h5F = if  drop c"  SOCK_PPOE       "
	thens
	.cstr
	drop
	...
	."  SMR: " _ip_smr@  .byte
	."    SCR: " _ip_scr@ .byte
	."  SPORT: " _ip_sport@ .word 
	."    SDIPR: " _ip_sdipr@ dup h18 rshift hFF and .byte h2E emit 
		dup h10 rshift hFF and .byte  h2E emit dup h8 rshift hFF and .byte  h2E emit hFF and .byte
	."  SDPORT: " _ip_sdport@ .word cr
	."  TX_FSR: " _ip_stx_fsr@ .word
	."            TX_RD: " _ip_stx_rd@ .word
	."              TX_WR: " _ip_stx_wr@ .word cr
	."  RX_RSR: " _ip_srx_rsr@ .word
	."            RX_RD: " _ip_srx_rd@ .word cr
	."   RXoff: " _ip_rxoffset W@ .word
	."          RXcount: " _ip_rxcount W@ .word cr
	."   TXoff: " _ip_txoffset W@ .word
	."          TXcount: " _ip_txcount W@ .word
	."         sockstatus: " _ip_sockstatus W@ .word
	."             TXLoop: " _ip_txloop C@ .byte
	cr

	decimal

	." SETTINGS:~h0D"
	."     listenport: " _ip_listenport W@ .word cr
	."       destsport: " _ip_destport W@ .word cr
	."     destipaddr: " _ip_destipaddr L@
		dup h18 rshift	hFF and .byte h2E emit
		dup h10 rshift	hFF and .byte h2E emit
		dup  h8 rshift	hFF and .byte h2E emit
				hFF and .byte cr
	xdbg
;
...
\ ( addr newcount cntdiff -- )
	...
	st?
	xdbg
	base W@ _ip_sockstat1 _ip_sockstat2 base W!
	>dbg cr xdbg
;
...
[ifdef ip_debug
		...
		if
			...
			." ~h0D001 "
			dup emit
			."  _ip_txoffset _ip_txcount: "
			_ip_txoffset W@ . _ip_stx_wr@ . _ip_txcount W@ .
			st?
			xdbg
		then
]
...
\ >dbg ." IAC RECEIVED "dup base W@ swap hex .word base W! xdbg
...
\ >dbg dup base W@ swap hex .word cr base W! xdbg
				dup
				if
...
\ dup 0<> if 7lock dup .con cr 7unlock then
	if
		_ip_si
\ dup 0<> if 7lock dup .con cr 7unlock then
		drop
