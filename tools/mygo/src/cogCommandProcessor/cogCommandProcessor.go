...
func (cp *CogCommandProcessor) CommandProcessor( scriptName string, fromCog chan byte, toCog chan byte, dtr chan bool, log chan string) chan byte {
...
type CogCommandProcessor struct {
	...
	chanDTR, chanReceiveTimeout, chanEmitQuit, chanUnsave chan bool

	receiveDelay time.Duration
	cps int64
	binary, echo, passThrough, expandCR, xlateLFtoCR bool
...
func (cp *CogCommandProcessor) GetReceiveTimeout() time.Duration{
	return cp.receiveDelay
}
...
func (cp *CogCommandProcessor) processAppend() (string, *os.File){
	var fp *os.File
	var name string
	var e error
	...
	if len(cp.fields) < 2 {
		...
		name = cp.fields[1]
		fp, e = os.OpenFile(name,os.O_RDWR | os.O_APPEND | os.O_CREATE,0666)
		if e != nil {
...
func (cp *CogCommandProcessor) processScriptFile() {
	...
	if fp != nil {
		...
		nc := np.CommandProcessor( name, cp.chanFromCog, cp.chanToCog, cp.chanDTR, cp.chanConsoleLog)
		np.logFile = cp.logFile
		np.resultFile = cp.resultFile
		np.resultFileName = cp.resultFileName
		np.logFileName = cp.logFileName
		np.echo = cp.echo
		np.expandCR = cp.expandCR
		np.xlateLFtoCR = cp.xlateLFtoCR
		np.cps = cp.cps
	...
	for _,v := range(cp.fields[1:]) {
		...
		if fp != nil {
			...
			go func ( p *CogCommandProcessor, s string, chanDone chan bool) {
				...
				nc := np.CommandProcessor( name, dummyFromCog, dummyToCog, p.chanDTR, p.chanConsoleLog)
				np.echo = p.echo
				np.cps = p.cps
...
func (cp *CogCommandProcessor) appendLogFile() {
	var fp * os.File
	cp.logFileName,fp = cp.processAppend( )
	if fp != nil {
		if cp.logFile != nil {
			cp.logFile.Close()
		}
...
func (cp *CogCommandProcessor) appendResultFile() {
	var fp * os.File
	cp.resultFileName,fp = cp.processAppend()
	if fp != nil {
		if cp.resultFile != nil {
			cp.resultFile.Close()
		}
...
func (cp *CogCommandProcessor) appendOutputFile() {
	var fp * os.File
	cp.outputFileName,fp = cp.processAppend()
	if fp != nil {
		if cp.outputFile != nil {
			cp.outputFile.Close()
		}
...
func (cp *CogCommandProcessor) logDelays( ){
	cp.logOutput( fmt.Sprintf( " %d cps  Receive Delay %v Echo: %v Expand CR->CRLF: %v Translate LF_CR: %v Binary: %v\n",
		cp.cps, cp.receiveDelay, cp.echo, cp.expandCR, cp.xlateLFtoCR, cp.binary))
}
...
func (cp *CogCommandProcessor) waitEmitNotRunning() {
	for ;cp.chanEmitQuit != nil; {
		time.Sleep(1e8)
	}
}
func (cp *CogCommandProcessor) emitQuit() {
	cp.chanEmitQuit <- true
	cp.waitEmitNotRunning()
}
func (cp *CogCommandProcessor) emitLogger() {
	go func() {

		cp.logOutput( "STARTING SCRIPT FILE EMIT LOGGER")
		cp.chanEmitQuit = make( chan bool)

		d := make([]byte, 0, 2048)
		
		filterQuit := make( chan bool)		
		filterDone := make( chan bool)		
		fromCog := make( chan byte);
		go func() {
			for q:= false; q == false; {
				select {
		...
		for q:= false; q == false; {
			...
			case c := <-fromCog:
				d = append( d, c)
				if c == 0x0D && cp.expandCR {
					d = append(d, 0x0A)
				}
				if len(d) == cap(d) || (len(d) > 1 && d[len(d)-1] == byte(0x0D)) ||
					( len(d) > 2 && d[len(d)-2] == byte(0x0D) && d[len(d)-1] == byte(0x0A)) {
					d = cp.emitter( d)
				}
		...
		filterQuit <- true
		cp.chanEmitQuit = nil
		cp.logOutput( "ENDING SCRIPT FILE EMIT LOGGER")
	}()
}
...
func (cp *CogCommandProcessor) doWait() {
	cp.logOutput( "WAIT STARTED")
	cp.Lock()
	cp.chanReceiveTimeout = make( chan bool)
	cp.Unlock()
	cp.chanEmitQuit <- false
	...
	for ; cp.getLine(); {
		for ; len(cp.fields) > 0; {
			...
			case "SXW":
				cp.echoHexWords()
			case "SDS":
				str := time.Now().Local().Format("2006Jan02 15:04")
			...
			case "c":
				cp.expandCR = cp.parseInt64() != int64(0)
			case "d":
				cp.xlateLFtoCR = cp.parseInt64() != int64(0)
			case "e":
			...
			case "t":
				...
				cp.receiveDelay = time.Duration( rd * 1e6)
			case "\x07":
				cp.passThrough = true
				cp.fields = cp.fields[0:0]
			case "ipc":
